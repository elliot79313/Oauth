...
public class Oauth  {
	...
	public static boolean setService(Oauth userinfo,HttpServletRequest req){
		...
		if(userinfo.provider.equals(Oauth.PROVIDER.Facebook.toString())){
			...
			fbsetOauth.setUID(userinfo.getUid());
    	...
    	queue.add(withUrl("/api/checkuser").method(Method.POST).param("uid", userinfo.getUid()).param("actoken", userinfo.getAc_token())
